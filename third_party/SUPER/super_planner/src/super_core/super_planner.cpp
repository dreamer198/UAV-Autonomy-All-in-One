...
namespace super_planner {
    ...
    void SuperPlanner::reset() {
        std::lock_guard<std::mutex> guard(replan_lock_);
        cmd_traj_info_.clear();
        last_exp_traj_info_.setEmpty();
        robot_on_backup_traj_ = false;
        on_backup_start_WT = -1.0;
        on_backup_end_WT = -1.0;
        local_start_p_.setZero();
        shifted_sfc_start_pt_.setZero();
        gi_.goal_p.setZero();
        gi_.goal_yaw = NAN;
        gi_.new_goal = true;
    ...
    SuperPlanner::PlanFromRest(const Vec3f &goal_p,
        ...
        if (back_ret_code == SUCCESS) {
            ...
            if (!cmd_traj_info_.setTrajectory(exp_traj_info, back_traj_info)) {
                ros_ptr_->error(" -- [SUPER] in [PlanFromRest] failed to commit trajectory.");
                return FAILED;
            }
    ...
    SuperPlanner::ReplanOnce(const Vec3f &goal_p,
        ...
        if (back_ret_code == SUCCESS) {
            if (!cmd_traj_info_.setTrajectory(exp_traj_info, back_traj_info)) {
                ros_ptr_->error(" -- [SUPER] in [ReplanOnce] failed to commit trajectory.");
                return FAILED;
            }
            last_exp_traj_info_ = exp_traj_info;
            robot_on_backup_traj_ = false;
    ...
    bool SuperPlanner::getOneHeartbeatTime(double &start_WT_pos,
                                           bool &traj_finish,
        ...
        trajectory_id = cmd_traj_info_.getTrajectoryId();
        cmd_traj_info_.unlock();
        return true;
    }

    bool SuperPlanner::getCommittedTrajectory(Trajectory &position,
                                              Trajectory &yaw,
                                              uint32_t &trajectory_id) {
        cmd_traj_info_.lock();
        if (cmd_traj_info_.empty()) {
        ...
        position = cmd_traj_info_.posTraj();
        yaw = cmd_traj_info_.yawTraj();
        trajectory_id = cmd_traj_info_.getTrajectoryId();
        cmd_traj_info_.unlock();
        return true;
    }
    ...
    bool SuperPlanner::getOneCommandFromTraj(StatePVAJ &pvaj,
                                             double &yaw,
                                             double &yaw_dot,
                                             bool &on_backup_traj,
                                             bool &traj_finish,
