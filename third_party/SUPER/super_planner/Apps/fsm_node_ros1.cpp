...
#define CONFIG_FILE_DIR(name) (string(string(ROOT_DIR) + "config/"+name))
    std::string dft_cfg_path = CONFIG_FILE_DIR("click_smooth_ros1.yaml");
    std::string cfg_path, cfg_name;
    if (pnh.param("config_path", cfg_path, dft_cfg_path)) {
        cout << " -- [Fsm-Test] Load config from: " << cfg_path << endl;
    } else if(pnh.param("config_name", cfg_name, dft_cfg_path)){
