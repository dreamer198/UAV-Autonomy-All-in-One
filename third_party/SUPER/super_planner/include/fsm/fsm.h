...
namespace fsm {
    ...
    protected:
        ...
        std::atomic<uint64_t> lifecycle_epoch_{1};
        std::atomic<uint8_t> progress_state_{0};
    ...
    public:
        ...
        void callPlanOnce(const Vec3f &goal) {
            ...
            std::string reason;
            if (!setGoalPosiAndYaw(goal, q, effective_goal, effective_yaw, reason)) {
                return;
            }
        ...
        uint8_t getProgressState() const {
            return progress_state_.load(std::memory_order_acquire);
        }

        uint64_t getLifecycleEpoch() const {
            return lifecycle_epoch_.load(std::memory_order_acquire);
        }

        bool resetLifecycle(std::string &reason);
    ...
    protected:
        ...
        bool validateGoalPosiAndYaw(const Vec3f &p,
                                    const Quatf &q,
                                    Vec3f &effective_goal,
                                    double &effective_yaw,
                                    std::string &reason);
        ...
        virtual void publishProgressState(uint8_t progress_state) = 0;
    };
}
