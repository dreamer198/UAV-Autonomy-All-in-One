...
void se3Ctrl::setDesiredStateToCurrentOdom()
{
    ...
    desire_odom_.header.stamp = ros::Time::now();
    ...
    desire_odom_.twist.twist.linear.x = 0.0;
    desire_odom_.twist.twist.linear.y = 0.0;
    desire_odom_.twist.twist.linear.z = 0.0;
}
...
void se3Ctrl::multiDOFJointCallback(const trajectory_msgs::MultiDOFJointTrajectory &msg) 
{
    ...
    if (use_acceleration_feedforward_ && !pt.accelerations.empty()) {
        desired_state_.a(0) = pt.accelerations[0].linear.x;
        desired_state_.a(1) = pt.accelerations[0].linear.y;
        desired_state_.a(2) = pt.accelerations[0].linear.z;
        for (int i = 0; i < 3; ++i) {
            desired_state_.a(i) = std::max(std::min(desired_state_.a(i), max_feedforward_acc_), -max_feedforward_acc_);
        }
    } else {
        desired_state_.a.setZero();
    }
    ...
    desired_state_.yaw = utils::fromQuaternion2yaw(desired_state_.q);
    desired_state_.yaw_rate = use_yaw_rate_feedforward_ ? pt.velocities[0].angular.z : 0.0;
}
