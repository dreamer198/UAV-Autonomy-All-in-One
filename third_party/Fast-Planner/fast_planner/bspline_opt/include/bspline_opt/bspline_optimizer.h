...
// B-spline 后端优化器：以控制点为变量，用 NLopt 最小化平滑、避障、动力学等代价。
...
public:
  ...
  // 输入控制点矩阵和 knot 间隔，输出优化后的控制点矩阵。
  Eigen::MatrixXd BsplineOptimizeTraj(const Eigen::MatrixXd& points, const double& ts,
                                      const int& cost_function, int max_num_id, int max_time_id);

  /* helper function */

  // required inputs：每次优化必须提供的控制点、时间间隔、代价组合和终止条件。
  ...
  // optional inputs：topo 引导路径或指定航点约束。
...
private:
  ...
  /* optimization parameters：各类代价权重、动力学限制和 NLopt 算法配置。 */
  ...
  /* 缓存每个代价项的梯度，避免 NLopt 迭代中频繁分配释放内存。 */
  ...
  /* cost function */
  /* 下面这些函数分别计算单项代价和梯度，combineCost 会按位标志组合它们。 */
