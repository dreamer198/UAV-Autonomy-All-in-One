...
// 分段多项式轨迹表示。topo 模式用它保存 min-snap 全局参考轨迹，
// 之后再从参考轨迹中截取局部段转成 B-spline。
...
private:
  ...
  /* evaluation：缓存离散采样结果，便于统计长度/速度和可视化。 */
...
public:
  ...
  vector<Eigen::Vector3d> getTraj() {
    // 以 0.01s 间隔离散整条轨迹，主要用于 RViz 可视化和长度统计。
    double eval_t = 0.0;
    traj_vec3d.clear();
