...
// 简单可视化工具：把离散轨迹点画成 RViz sphere list。
...
void odomCallbck(const nav_msgs::Odometry& msg) {
  ...
  // 只用 odom 初始化轨迹起点，后续轨迹按生成结果开环发布。
  odom = msg;
  have_odom = true;
}

int main(int argc, char** argv) {
  /* 示例节点：生成一条硬编码航点多项式轨迹，并向外发布位置/速度/加速度命令。 */
  ...
  /* 构造示例航点和每段时间，使用闭式 minimum jerk/min-snap 生成多项式。 */
  ...
  /* 将系数矩阵转换成 PolynomialTraj，方便按时间求值和可视化。 */
