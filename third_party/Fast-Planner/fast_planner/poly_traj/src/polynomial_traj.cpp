...
PolynomialTraj minSnapTraj(const Eigen::MatrixXd& Pos, const Eigen::Vector3d& start_vel,
  ...
  // 根据航点和每段时间闭式求解分段五次多项式，使整体 jerk/snap 代价最小。
  // topo 模式用这条轨迹作为全局参考线，不一定直接执行。
  ...
  /* Q 矩阵：积分三阶导数代价。这里 i 从 3 开始，因此实际惩罚的是 jerk。 */
  ...
  /* 闭式解：固定导数已知，中间自由导数通过二次型最优条件直接求出。 */
  ...
  /* 将求得的 x/y/z 多项式系数打包成 PolynomialTraj。 */
...
PolynomialTraj fastLine4deg(Eigen::Vector3d start, Eigen::Vector3d end, double max_vel, double max_acc,
                            double max_jerk) {
  // 生成一条受速度/加速度/jerk 限制的直线分段多项式轨迹，主要用于简单测试。
