...
// 用哈希表管理已发现节点，避免在 open/close set 中线性查找。
...
// 几何 A* 搜索器：在 3D 栅格地图中找一条无碰撞路径。
// 相比 KinodynamicAstar，它更简单，适合作为全局/辅助路径或调试基线。
class Astar {
private:
  /* ---------- main data structure ---------- */
  ...
  // 从终点沿 parent 回溯得到完整路径。
  void retrievePath(NodePtr end_node);

  /* heuristic function：不同距离度量的启发式函数。 */
