...
// 多段路径组合枚举器：当一条路径由多个可选片段组成时，遍历所有组合。
class TopoIterator {
private:
  /* data */
...
/* ---------- node of topo graph ---------- */
// 拓扑图节点分两类：
// Guard 是可见区域代表点，Connector 是连接两个 Guard 的桥接点。
class GraphNode {
private:
  /* data */
...
// TopologyPRM 负责从碰撞区间两端寻找多条拓扑不同的绕障路径。
// PlannerManager 会用这些路径作为 B-spline 优化的 guide path。
class TopologyPRM {
private:
  /* data */
  ...
  // raw_paths_: DFS 找到的原始图路径；short_paths_: shortcut 后的路径；
  // final_paths_: 去重和筛选后的候选路径。
  ...
  // 整体流程：采样建图 -> DFS 搜索路径 -> shortcut 变短 -> 去掉拓扑等价路径 -> 选短路径。
