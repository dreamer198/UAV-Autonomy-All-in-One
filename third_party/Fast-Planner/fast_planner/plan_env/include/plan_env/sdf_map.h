...
// 建图模块的参数集合：地图尺寸、深度相机模型、raycasting 概率模型和可视化配置。
...
// 建图运行时数据：包括 occupancy/ESDF 缓存、相机状态、深度投影点和更新标志。
...
public:
  ...
  // occupancy map 管理：世界坐标/栅格索引转换、占据状态读写、局部缓存清理。
  void resetBuffer();
  ...
  // ESDF 距离场管理：查询距离/梯度、更新 3D ESDF、取切片用于可视化。
  ...
  // 初始化 ROS 参数、订阅/发布器、定时器和地图缓存。
  void initMap(ros::NodeHandle& nh);

  // RViz 调试输出：占据点云、膨胀障碍、ESDF 切片、局部更新范围等。
  void publishMap();
...
private:
  ...
  // 输入回调：支持 depth+pose、depth+odom 或 pointcloud+odom 三类数据路径。
  ...
  // 定时更新：先 raycasting 融合 occupancy，再根据局部更新范围计算 ESDF。
  void updateOccupancyCallback(const ros::TimerEvent& /*event*/);
  void updateESDFCallback(const ros::TimerEvent& /*event*/);
  void visCallback(const ros::TimerEvent& /*event*/);

  // 建图主流程：深度图投影、射线融合、清理/膨胀局部障碍。
