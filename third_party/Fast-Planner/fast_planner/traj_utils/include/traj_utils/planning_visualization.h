...
// 规划可视化工具类：把路径、B-spline、拓扑图、预测轨迹和 yaw 方向发布成 RViz Marker。
class PlanningVisualization {
private:
  // 同一类 marker 使用固定 id 段，便于更新时先删除旧 marker 再画新 marker。
...
public:
  ...
  // draw basic shapes：底层 marker 封装，其他绘图函数都基于这几个接口。
  ...
  // draw a bspline trajectory，可选显示控制点。
  ...
  // draw topological graph and paths：用于观察 topo PRM 的图和候选路径。
