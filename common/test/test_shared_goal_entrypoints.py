...
class SharedGoalEntrypointTest(unittest.TestCase):
    ...
    def test_both_launchers_stream_the_shared_executor(self):
        for launcher in ("launch/sim.sh", "launch/real.sh"):
            with self.subTest(launcher=launcher):
                ...
                self.assertIn("GOAL_EXECUTOR_HOST=", source)
                self.assertIn('< "$GOAL_EXECUTOR_HOST"', source)
                self.assertNotIn("rostopic pub -1 /goal", source)

    def test_shared_executor_has_no_platform_branch(self):
        source = self.read_package_file("scripts/goal_executor.py")
