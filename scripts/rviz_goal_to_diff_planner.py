...
class RvizGoalToDiffPlanner:
    def __init__(self):
        ...
        rospy.Subscriber(
            "/move_base_simple/goal", PoseStamped, self.nav_goal_cb, queue_size=1
        )
        rospy.Subscriber("/clicked_point", PointStamped, self.clicked_point_cb, queue_size=1)

        rospy.loginfo(
            "rviz_goal_to_diff_planner started: /move_base_simple/goal and "
            "/clicked_point -> %s, default_z=%.3f",
            self.output_topic,
            self.default_z,
        )
    ...
    def clicked_point_cb(self, msg):
        ...
        rospy.loginfo(
            "Publish Point -> %s: x=%.3f y=%.3f z=%.3f frame=%s",
            self.output_topic,
            out.pose.position.x,
            out.pose.position.y,
            out.pose.position.z,
